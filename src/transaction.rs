...
impl Transaction {
    pub fn new(
        ...
        if amount <= 0.0 {
            return Err("مقدار transaction باید مثبت باشد".to_string());
        }
    ...
    pub fn new_with_fee(
        ...
        let mut transaction = Transaction {
            ...
            from_address: from_address.clone(),
            to_address,
            amount: amount + fee, // Add fee به مقدار
            timestamp: chrono::Utc::now(),
            signature: None,
        };
    ...
    pub fn sign_transaction(&mut self, wallet: &crate::address::Wallet) -> Result<(), String> {
        if self.from_address != wallet.get_address() {
            return Err("شما نمی‌توانید transaction‌های دیگران را امضا کنید".to_string());
        }

        let hash = self.calculate_hash();
        // Simplified signature for compatibility
    ...
    pub fn sign_transaction_simple(&mut self, wallet: &crate::address::Wallet) -> Result<(), String> {
        ...
        if wallet.private_keys.is_empty() {
            return Err("کلید خصوصی not found".to_string());
        }
    ...
    pub fn is_valid(&self) -> bool {
        // Check transaction Genesis (بدون امضا)
    ...
    fn verify_signature(&self) -> bool {
        if let Some(signature_str) = &self.signature {
            ...
            // برای سادگی، فعلاً true برمی‌گRejectانیم
    ...
    // transaction Genesis برای اولین block
...
impl fmt::Display for Transaction {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "💸 transaction: {} -> {} | مقدار: {} RustCoin | زمان: {}",
